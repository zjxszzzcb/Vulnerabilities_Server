<template>
  <el-form ref="ruleFormRef" :rules="rules" :model="formFood"  label-width="80px">
    ...
    <el-form-item label="菜品图片" style="margin: auto;">
      <el-upload ref="upload1" class="icon-uploader" name="file" :show-file-list="false" :on-change="fileChange1" :http-request="uploadIcon" :limit="1">
        ...
        <span style="margin-left: 10px; font-size: 10px; text-decoration: underline;line-height: 20px;">点击上传</span>
      </el-upload>
    </el-form-item>
    </el-col>
    <el-col :span="12">
  ...
  <div class="dialong__button--wrap">
    ...
    <el-button color="#178557" :loading="subLoading" type="success" @click="addFood(ruleFormRef)">保存</el-button>
  </div>
</template>

<script setup lang="ts">
import { reactive,ref } from 'vue'
import { addFoodApi, upfoodicon, upfoodvideo } from "../../../api/food/food"
import type { FormInstance, FormRules } from 'element-plus'
import TextEditor from './TextEditor.vue'
import { ElMessage } from 'element-plus'
import { UploadRequestOptions } from "element-plus/lib/components";

const emit = defineEmits(['closeAddFoodForm','success'])
...
// 新增菜品信息
const addFood = async (formEl: FormInstance | undefined) => {
  if (!formEl) return
  await formEl.validate(async (valid, fields) => {
    subLoading.value = true
    if (valid) {
      const { data } =  await addFoodApi(formFood)
      if(data.code===200){
...
// 设置icon文件上传请求
const uploadIcon = async (params: UploadRequestOptions) =>{
  if (!params) return
  loading1.value = true
  // 判断上传文件类型
