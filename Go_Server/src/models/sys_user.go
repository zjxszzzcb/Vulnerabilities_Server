...
type SysUser struct {
	...
	// 备注
	Remarks string `gorm:"column:remarks;type:longtext;" json:"remarks"`
}
...
func GetUserList(keyword string, Status int) *gorm.DB {
	tx := DB.Model(new(SysUser)).Select("sys_user.id,sys_user.role_id,sr.name role,sys_user.username,sys_user.password,sys_user.phone,sys_user.sex,sys_user.email,sys_user.avatar,sys_user.status,sys_user.created_at,sys_user.updated_at").Joins("LEFT JOIN sys_role sr ON sr.id = sys_user.role_id")
	if keyword != "" {
