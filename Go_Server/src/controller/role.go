...
func GetRoleList(c *gin.Context) {
	...
	if err := models.GetRoleList(in.Keyword).Offset((in.Page - 1) * in.Size).Limit(in.Size).Find(&list).Error; err != nil {
		helper.ErrorResponse(c, "获取角色列表信息", err)
		return
	}
	if err := models.GetRoleList(in.Keyword).Count(&cnt).Error; err != nil {
		helper.ErrorResponse(c, "获取角色列表信息", err)
		return
	}
...
func AddRole(c *gin.Context) {
	...
	var cnt int64
	// 大于0说明存在用户
	if err := models.DB.Model(new(models.SysRole)).Where("name = ?", in.Name).Count(&cnt).Error; cnt > 0 && err == nil {
