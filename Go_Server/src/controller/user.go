...
func GetUserList(c *gin.Context) {
	...
	if err := models.GetUserList(in.Keyword, in.Status).Offset((in.Page - 1) * in.Size).Limit(in.Size).Find(&list).Error; err != nil {
		helper.ErrorResponse(c, "获取用户列表信息", err)
		return
	}
	if err := models.GetUserList(in.Keyword, in.Status).Count(&cnt).Error; err != nil {
		helper.ErrorResponse(c, "获取用户列表信息", err)
		return
	}
