...
func GetFoodList(c *gin.Context) {
	...
	// 获取食物数据列表
	if err := models.GetFoodList(in.Keyword).Offset((in.Page - 1) * in.Size).Limit(in.Size).Find(&list).Error; err != nil {
		helper.ErrorResponse(c, "获取食物数据列表", err)
		return
	}
	// 获取食物数据总数
	if err := models.GetFoodList(in.Keyword).Count(&cnt).Error; err != nil {
		helper.ErrorResponse(c, "获取食物数据总数", err)
		return
	}
